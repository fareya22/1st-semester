...
void edgeCreating(int SL, int EL, bool startExecution) {
    int node = SL;
    while (node <= EL) {
        if (startExecution) {
            if (ifline(node)) {
                ...
                if (struct_info[id].keyword == "for" || struct_info[id].keyword == "while") {
                    ...
                    edgeCreating(struct_info[id].line_begin + 1, struct_info[id].end_line - 1, true);
...
void analyze(int startLine, int endLine) {
    ...
    bool startExecution = false; // Flag to track whether execution should start
    ...
    for (int LN = startLine; LN <= endLine; ++LN) {
        for (int CL = 0; CL < word[LN].size(); ++CL) {
            ...
            // Check for the main() function to start execution
            if (word[LN][CL] == "int" && word[LN][CL + 1] == "main" && word[LN][CL + 2] == "(" && word[LN][CL + 3] == ")") {
                edgeCreating(startLine, endLine, true);
                return;
            }
        }
    }

    edgeCreating(startLine, endLine, false);
}
...
void cfg() {
    ...
    if (mainStartLine != -1) {
        edgeCreating(mainStartLine, totalLine, true);
        printGraph();
    } else {
